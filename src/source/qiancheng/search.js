...
module.exports = class Search{
  ...
  */
   ...
   * 
   * @param {String} event 
   * @param {Function} callback // callback 如果返回true ，则继续爬取下一页，false则停止爬取
   * 接受三个参数： [err, index, html]
   */
    on(event, callback) {
        if (typeof callback != 'function') return
        if (event == 'each-list-page') {
            this.$eachListPageCallback = callback
        }
    }
    async getListpage(){
        let jobs = []
        ...
        while(1){
            let url = this.generateUrl(this.searchKey, 1),
                ...
                pageJob = await this.handleListpage(html),
                jobs = jobs.concat(pageJob)
            this.pageCount ++
            if (this.$eachListPageCallback){
                if (!this.$eachListPageCallback(null, this.pageCount, pageJob)) break
            } else {
